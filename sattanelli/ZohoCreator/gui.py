...
def zohoCreatorGUI(token):
    ...
    def getJobRecord():
        ...
        id_record, note_record = zoho_API.getSimpleRecord(text, job_url, refresh_token)
        #label_job.configure(text = id_record + ' ' + note_record)

        #record completo
        url = 'https://creator.zoho.com/api/v2/' + user + '/' + app + '/report/' + report
        job_url = url + '?criteria=JobID=="' + entry1.get() + '"'
        labels = [label_job1, label_job2, label_job3, label_job4, label_job5]
        zoho_API.getCompleteRecord(text, labels, job_url, refresh_token)
    
    def getAllSerialsTEST():
        global id_record
        if refresh_token == None:
            genToken()
        serials_report = 'All_Serials'
        url = 'https://creator.zoho.com/api/v2/' + user + '/' + app + '/report/' + serials_report
        job_url = url + '?criteria=REfJobID.contains("' + entry1.get() + '")'
        #job_url = 'https://creator.zoho.com/api/v2/bennice2/tabscomuni1/report/All_Serials?criteria=REfJobID.contains("NE20020")'  #COSì FUNZIONA
        print(job_url)
        json = zoho_API.getRecordTest(text, job_url, refresh_token)
        pnt('--------------------------------------------------------------------------------')
        pnt(json)
        url = 'https://creator.zoho.com/api/v2/' + user + '/' + app + '/report/' + 'All_Serials' + '/3343883000000762018'
        #zoho_API.updateRecordTest(text, url, refresh_token, json)
    ...
    def selected(*args):
        ...
        while(i != len(prj_status_array)):
            ...
            if(arr[0] == str(variable.get())):
                ...
                status_name = arr[0]
                text.insert(INSERT,'New Status: ' + status_name + '\n')
            i += 1
    ...
    image_menu5 = PhotoImage(file = internal_link + 'img/' + 'cil-menu.png')
    ...
    # TITOLO SCHERMATA
    label_space = Label(GUI, text = "", font = font2, width = 147, height = 1, fg = "white", background = "#253b3a")
    label_job = Label(GUI, text = "", image = image_entry1, bd=0)
    label_job1 = Label(GUI, text = "", font = font2, width = 100, height = 4, wraplength=700, borderwidth=2, anchor=W, relief="flat", fg = "yellow", background = "#404040")
    label_job2 = Label(GUI, text = "", font = font2, width = 100, height = 4, wraplength=700, borderwidth=2, anchor=W, relief="flat", fg = "white", background = "#404040")
    label_job3 = Label(GUI, text = "", font = font2, width = 100, height = 4, wraplength=700, borderwidth=2, anchor=W, relief="flat", fg = "white", background = "#404040")
    label_job4 = Label(GUI, text = "", font = font2, width = 100, height = 4, wraplength=700, borderwidth=2, anchor=W, relief="flat", fg = "white", background = "#404040")
    label_job5 = Label(GUI, text = "", font = font2, width = 100, height = 4, wraplength=700, borderwidth=2, anchor=W, relief="flat", fg = "white", background = "#404040")
    ...
    button_output = HoverButton(GUI, text = "      updateRecordID", command = updateRecordID, image = image_menu2, compound=LEFT, width=180, height=62, bg='#292626', fg='#ffffff', activebackground='#6D6F89', activeforeground='#ffffff', bd = 0)
    button_job = HoverButton(GUI, text = "     Import Job Data", command = getJobRecord, image = image_menu3, compound=LEFT, width=180, height=62, bg='#292626', fg='#ffffff', activebackground='#6D6F89', activeforeground='#ffffff', bd = 0)
    button_exit = HoverButton(GUI, text = "            Chiudi          ", command = exit, image = image_menu4, compound=LEFT, width=180, height=62, bg='#292626', fg='#ffffff', activebackground='#6D6F89', activeforeground='#ffffff', bd = 0)
    button_test = HoverButton(GUI, text = "     TestSerials", command = getAllSerialsTEST, image = image_menu3, compound=LEFT, width=180, height=62, bg='#292626', fg='#ffffff', activebackground='#6D6F89', activeforeground='#ffffff', bd = 0)
    button_test2 = HoverButton(GUI, text = "     TestUpdate", command = getAllSerialsTEST, image = image_menu3, compound=LEFT, width=180, height=62, bg='#292626', fg='#ffffff', activebackground='#6D6F89', activeforeground='#ffffff', bd = 0)
    ...
    button_test['font'] = font1
    button_test2['font'] = font1
    ...
    button_output.grid(column = 1, row = 4)
    button_job.grid(column = 1, row = 3)
    button_exit.grid(column = 1, row = 5)
    button_test.grid(column = 4, row = 3)
    button_test2.grid(column = 4, row = 3)
    ...
    text = Text(frame, width=105, height=24, wrap=WORD, bd=0, xscrollcommand=xscrollbar.set, yscrollcommand=yscrollbar.set, fg = "#4bc973", background = "#292626")
    ...
    entry1.insert(INSERT, 'TEST_ITC') #[Insert Job]
    entry1.focus()
