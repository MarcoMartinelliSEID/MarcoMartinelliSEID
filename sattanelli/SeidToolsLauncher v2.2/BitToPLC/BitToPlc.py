...
def openBitToPLC():
    ...
    def readIOlist():
        ...
        x = Gestore.smartX(text, ioList)
        descr_length = spin1.get()
        AI, AO, DI, DO, SAI, SAO, SDI, SDO, RAI, RAO, RDI, RDO, RSAI, RSAO, RSDI, RSDO, RTD = ([] for i in range(17))
        Gestore.readSignals(ioList, text2, x, column_data, descr_length, AI, AO, DI, DO, SAI, SAO, SDI, SDO, RAI, RAO, RDI, RDO, RSAI, RSAO, RSDI, RSDO, RTD)
        Gestore.printOp(text, AI, AO, DI, DO, SAI, SAO, SDI, SDO, RAI, RAO, RDI, RDO, RSAI, RSAO, RSDI, RSDO, RTD)
        label_file_explorer5.configure(text='\n' +
            'AI =  ' + str(len(AI)) + '\t\t' + 'SAI =  ' + str(len(SAI)) + '\t\t' + 'RAI =  ' + str(len(RAI)) + '\t\t' + 'RSAI =  ' + str(len(RSAI)) + '\t\t' +
            'RTD =  ' + str(len(RTD)) + '\n' +
            'AO =  ' + str(len(AO)) + '\t\t' + 'SAO =  ' + str(len(SAO)) + '\t\t' + 'RAO =  ' + str(len(RAO)) + '\t\t' + 'RSAO =  ' + str(len(RSAO)) + '\t\t' + '\n' +
            'DI =  ' + str(len(DI)) + '\t\t' + 'SDI =  ' + str(len(SDI)) + '\t\t' + 'RDI =  ' + str(len(RDI)) + '\t\t' + 'RSDI =  ' + str(len(RSDI)) + '\t\t\t' + '\n' +
            'DO =  ' + str(len(DO)) + '\t\t' + 'SDO =  ' + str(len(SDO)) + '\t\t' + 'RDO =  ' + str(len(RDO)) + '\t\t' + 'RSDO =  ' + str(len(RSDO)) + '\t\t' + '\n')


    def ElaboraDati():
        global filename, fileBit, bits
        global filename2
        global column_data
        global ioList
        try:
            fileIoList = load_workbook(filename)
            # fileIoList = load_workbook("C:/Users/marcom/Desktop/PY Workspace/BIT PLC spac/IOlist.xlsx")
            # fileBit = load_workbook("C:/Users/marcom/Desktop/PY Workspace/BIT PLC spac/bitPLC.xlsx")
            ioList = fileIoList[sheetIOlist]
        except:
            text2.insert(INSERT, "Errore: (IOLIST) nessun file o sheet excel selezionati.\n")
            text2.see("end")
        ...
        Gestore.Elaboration(filename2, fileBit, bits, text, text2, label_file_explorer3, AI, AO, DI, DO, SAI, SAO, SDI, SDO, RAI, RAO, RDI, RDO, RSAI, RSAO, RSDI, RSDO, RTD)
    ...
    image_menu = PhotoImage(file = 'BitToPlc/img/' + '6.png')
    image_menu2 = PhotoImage(file = 'BitToPlc/img/' + '2.png')
    image_menu3 = PhotoImage(file = 'BitToPlc/img/' + '3.png')
    image_menu4 = PhotoImage(file = 'BitToPlc/img/' + '7.png')
    image_menu5 = PhotoImage(file = 'BitToPlc/img/' + '10.png')
